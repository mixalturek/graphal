...
\declaration{V~Praze dne 18.\,11.\,2009}
...
Nejjednodušší implementací chytrých ukazatelů je šablona \texttt{auto\_ptr} ze standardní kni\-hov\-ny STL. Bude stačit nebo bude potřeba silnější nástroj? Po požádání se \texttt{auto\_ptr} stává výhradním vlastníkem ukazatele a při své destrukci maže i předanou dynamickou paměť. Výhradní vlastník znamená, že v~jednom okamžiku může existovat právě jedna \texttt{auto\_ptr} reference na danou paměť.
...
V~objektově orientovaném návrhu aplikačního rozhraní je vždy dobré zavést kořen hierarchie tříd, který bude poskytovat, či pouze definovat, základní obecné operace nad všemi používanými objekty. V~jazyce Java se taková třída jmenuje \texttt{Object}, knihovna Qt zavádí \texttt{QObject}, \texttt{CObject} je k nalezení v knihovně MFC. Vyvíjená aplikace použije jméno \texttt{BaseObject}.
...
Pokud každá odvozená třída definuje svoji metodu \texttt{dump()} -- a ony ji definovat musí, protože je čistě virtuální -- může do předaného proudu vypsat informace o~sobě a o~objektech, které v~sobě uchovává. Mějme na\-přík\-lad jednoduchý demonstrační skript.
...
XML formát výstupu samozřejmě není nutný, byl použit pouze kvůli pře\-hled\-nos\-ti výpisu strukturovaných dat.
...
\caption[Dva možné přístupy k~ukládání pozic]{Dva možné přístupy k~ukládání pozic (kolečko označuje data pozice)}
...
Implementovaný debugging podporuje celkem čtyři níže uvedené způsoby obnovění běhu skriptu známé z~běžných vývojových prostředí.
...
Hlavní okno aplikace dědí ze třídy \texttt{QMainWindow} a používá standardní rozvržení sestávající z hlavního menu, nástrojových lišt a stavového řádku. Jedná se o~MDI aplikaci, centrální widget tvoří textové editory pro úpravu kódu, které se dají přepínat pomocí záložek.
...
Rodičovská třída \texttt{VisualizationConnector} definuje potřebné rozhraní, současně implementuje těla všech svých metod jako prázdná. Nejedná se tedy o čistě abstraktní třídu jako v minulých případech. Zabudované funkce pro vizualizace komunikují s~GUI výhradně s jejím pro\-střed\-nict\-vím, resp. jejího potomka. Pokus o~přístup k~oknu z~konzolové aplikace proto nezpůsobí žádnou chybu, pouze se zavolá prázdná metoda komunikačního objektu, a to ničemu nevadí.
...
\caption{Režije výjimek při návratu z funkce}
...
Soubory ukládající grafy používají strukturu uvedenou níže. Jedná se o~textový formát, od\-dě\-lo\-va\-čem může být libovolný počet bílých znaků (mezera, tabulátor, nový řádek).
